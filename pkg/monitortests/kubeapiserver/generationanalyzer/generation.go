	...
	objChangeFns := []func(obj, oldObj metav1.Object) []monitorapi.Interval{
		func(obj, oldObj metav1.Object) []monitorapi.Interval {
			...
			if generation > maxGenerationAllowed {
				intervals = append(intervals, getInterval(namespace, name, generation, obj, oldObj))
			}
...
func getInterval(namespace, name string, generation int64, obj, oldObj metav1.Object) monitorapi.Interval {
	var msg string
	...
	case *appsv1.Deployment:
		locator = monitorapi.NewLocator().DeploymentFromName(namespace, name)
		msg = fmt.Sprintf("Deployment generation too high: %d", generation)
	case *appsv1.DaemonSet:
		locator = monitorapi.NewLocator().DaemonSetFromName(namespace, name)
		msg = fmt.Sprintf("DaemonSet generation too high: %d", generation)
	case *appsv1.StatefulSet:
		locator = monitorapi.NewLocator().StatefulSetFromName(namespace, name)
